...
openrouter_api_key = get_config_val("openrouter", "api_key", "OPENROUTER_API_KEY", "")
openrouter_base_url = get_config_val("openrouter", "base_url", "OPENROUTER_BASE_URL", "https://openrouter.ai/api/v1")
try:
    if openrouter_api_key:
        ...
        print(f"DEBUG: Initialized OpenRouter Client with base_url={openrouter_base_url}")
    else:
...
async def call_openrouter_with_retry_async(
    ...
    if openrouter_client is None:
        print("Error: OpenRouter client not initialized. Check your API key.")
        return ["Error"] * config["candidate_num"]

    system_prompt = config["system_prompt"]
    temperature = config["temperature"]
    candidate_num = config["candidate_num"]
    max_completion_tokens = config.get("max_completion_tokens", config.get("max_output_tokens", 50000))
    response_text_list = []
    ...
    for attempt in range(max_attempts):
        try:
            openai_formatted = _convert_to_openai_format(current_contents)
            first_response = await openrouter_client.chat.completions.create(
                model=model_name,
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": openai_formatted}
                ],
                temperature=temperature,
                max_completion_tokens=max_completion_tokens,
            )
            ...
            context_msg = f" for {error_context}" if error_context else ""
            print(
                f"OpenRouter validation attempt {attempt + 1} failed{context_msg}: {error_str}. Retrying in {retry_delay} seconds..."
            )
            if attempt < max_attempts - 1:
                await asyncio.sleep(retry_delay)

    if not is_input_valid:
        print(f"Error: All {max_attempts} OpenRouter attempts failed{context_msg}. Returning errors.")
        return ["Error"] * candidate_num

    remaining_candidates = candidate_num - 1
    if remaining_candidates > 0:
        print(f"OpenRouter input validated. Generating remaining {remaining_candidates} candidates...")
        valid_openai_contents = _convert_to_openai_format(current_contents)
        tasks = [
        ...
        for res in results:
            if isinstance(res, Exception):
                print(f"Error generating subsequent OpenRouter candidate: {res}")
                response_text_list.append("Error")
            else:
                response_text_list.append(res.choices[0].message.content)
...
async def call_text_model_with_retry_async(
    ...
    """
    Dispatch to the correct text-generation backend based on model name.
    - deepseek/* or openrouter/* → OpenRouter
    - gemini/* → Gemini
    ...
    if any(prefix in model_lower for prefix in ["deepseek", "openrouter", "qwen", "meta-llama", "mistral"]):
        ...
        return await call_gemini_with_retry_async(
            model_name, contents, gemini_config, max_attempts, retry_delay, error_context
        )
        ...
        print(f"Unknown model prefix '{model_name}', defaulting to OpenRouter.")
        return await call_openrouter_with_retry_async(
            model_name, contents, config, max_attempts, retry_delay, error_context
        )
