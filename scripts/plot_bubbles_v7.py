#!/usr/bin/env python3
"""plot_bubbles_v7.py — Acoustic bubble charts per dataset (test set only), v7.

v7 changes over v6:
  1. Legend restored as independent per-chart (not merged), top-right,
     reduced sizing (fontsize 7, markerscale 0.6, handletextpad=0.5,
     columnspacing=0.5).
  2. Every bubble labelled with its percentage number (fontsize 6.5,
     color #333, centred). Bubbles <5% are skipped.

Kept from v5/v6:
  - Bubble alpha=0.55, edgecolors='none', crossing baseline.
  - Title: just dataset name.
  - No blue dots, node labels below baseline.
  - figsize (12,8), dpi=200, white bg, no grid.
"""
...
def draw_bubble_row(ax, matrix, emotions, row_label: str, seed_offset: int = 0):
    ...
    v7: Every bubble labelled with percentage number (fontsize 6.5, #333).
         Bubbles <5% are skipped.
    """
    n_emo_matrix = matrix.shape[0]
    MAX_BUBBLE = 1800
    LABEL_THRESHOLD = 0.05  # skip bubbles below 5%
    ...
    for bi, (node_label, node_x) in enumerate(
        ...
        for position, (ei, prop) in enumerate(items):
            ...
            # v5 bubble style: alpha=0.55, no border
            ax.scatter(
            ...
            # v7: Label EVERY bubble with percentage (skip <5%)
            if prop >= LABEL_THRESHOLD:
                pct = int(round(prop * 100))
                # Position label text: centred on the bubble
                ax.text(node_x + x_jitters[position], y_off,
                        f'{pct}%',
                        ha='center', va='center',
                        fontsize=6.5, fontweight='normal',
                        color='#333333', alpha=0.95, zorder=7)
...
def add_individual_legend(fig_or_ax, emotions, bbox_to_anchor=(0.99, 0.98),
                           loc='upper right'):
    """v7: Add an individual (per-chart) compact legend, top-right, reduced sizing.

    Parameters:
      fontsize 7, markerscale 0.6, handletextpad=0.5, columnspacing=0.5
    """
    legend_elements = []
    for emo in emotions:
        legend_elements.append(
            Line2D([0], [0], marker='o', color='w',
                   ...
                   label=EMOTION_ABBREV[emo])
        )
    leg = fig_or_ax.legend(
        handles=legend_elements, loc=loc,
        fontsize=7, ncol=len(emotions),
...
# ========================================================================
# Individual dataset figure (v7)
# ========================================================================

def plot_dataset_figure_v7(
    ...
    """Plot individual dataset figure (v7).

    v7 changes over v6:
      - Legend: individual, top-right, reduced sizing.
      - Every bubble gets a percentage label.
    """
    ...
    # v7: Individual legend — top-right, reduced sizing
    add_individual_legend(fig, emotions,
                          bbox_to_anchor=(0.99, 0.98),
                          loc='upper right')

    # Title: just dataset name (same as v6)
...
# ========================================================================
# Combined figure (v7: individual legends per dataset, not merged)
# ========================================================================

def plot_combined_v7(results: dict, output_path: str):
    """Plot combined figure with 3 datasets in a 3x2 grid + individual legends.

    v7: Each dataset row gets its own compact legend (not a single merged one).
        Every bubble labelled with percentage.
    """
    ...
    for row, (key, dname) in enumerate(available):
        ...
        # v7: Individual legend per dataset — placed in the 3rd column area
        # Create an inset axes for the legend
        pos_rms = ax_rms.get_position()
        # Position the legend to the right of the RMS subplot
        legend_ax = fig.add_axes(
            [pos_rms.x1 + 0.02, pos_rms.y0,
             0.06, pos_rms.y1 - pos_rms.y0],
            facecolor='none')
        legend_ax.axis('off')

        # Draw legend as colored markers + text
        n_emo = len(emotions)
        for ei, emo in enumerate(emotions):
            y_pos = n_emo - ei - 0.5
            color = EMOTION_COLORS[emo]
            legend_ax.scatter(0.3, y_pos / max(n_emo, 1), s=40,
                            c=[color], alpha=0.82, edgecolors='none', zorder=5)
            legend_ax.text(0.7, y_pos / max(n_emo, 1), EMOTION_ABBREV[emo],
...
def main():
    ...
    # ======== Plotting ========
    print(f"\n{'='*60}")
    print("Generating v7 figures (individual legends, percentage labels on all bubbles)...")
    ...
    for key in ['cbesd', 'fauaibo', 'iemocap']:
        if key in results:
            ...
            plot_dataset_figure_v7(
                f0_m, rms_m, emotions, name_map[key], n, path,
            )

    # Combined figure (v7: individual legends per dataset)
    if len(results) >= 2:
        combined_path = os.path.join(OUTPUT_DIR, 'fig_bubbles_v4_combined.png')
        plot_combined_v7(results, combined_path)
