"""Deploy plot_bubbles_v2.py to remote server, run it, and download results."""
import paramiko
import os
import sys
import time
...
REMOTE_SCRIPT = f'{REMOTE_DIR}/plot_bubbles_v2.py'
...
OUTPUT_FILES = [
    'fig_bubbles_v2_cbesd.png',
    'fig_bubbles_v2_fauaibo.png',
    'fig_bubbles_v2_iemocap.png',
    'fig_bubbles_v2_combined.png',
]
...
def main():
    ...
    cmd = (
        ...
        f"{PYTHON} plot_bubbles_v2.py"
    )
    rc = run(cmd, "Running plot_bubbles_v2.py", stream=True)
    print(f"\nExit code: {rc}")

    # Step 4: List output files
    print("\n--- Remote output ---")
    run(f"ls -lh {REMOTE_DIR}/*.png 2>/dev/null || echo 'No PNG files found'")
