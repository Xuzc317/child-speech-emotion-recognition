#!/usr/bin/env python3
"""plot_bubbles_v4.py — Acoustic bubble charts per dataset (test set only), v4.

Layout (Tsangko et al. 2026 Fig.1 style):
  - N rows (acoustic features) × 1 column (NOT per emotion)
  - Each row: horizontal baseline with 5 nodes (VERY_LOW ~ VERY_HIGH)
  - At each node: emotion bubbles stacked/alternating above & below the line
  - Bubble area ∝ proportion of that emotion's samples in that bin
  - Color fixed per emotion (legend on right side)
  - Annotations on large bubbles (≥10%)
...
# ═══════════════════════════════════════════════════════════════════
# Plotting (v4 layout — Tsangko et al. reference style)
# ═══════════════════════════════════════════════════════════════════

def plot_dataset_figure_v4(
    ...
    """Plot v4: N rows (acoustic features) × 1 column, Tsangko-style.

    Each row:
      - Horizontal baseline at y=0
      - 5 nodes along baseline (VERY_LOW ~ VERY_HIGH)
      - At each node: emotion bubbles stacked alternating above/below
      - Bubble area ∝ proportion of that emotion in that bin
      - Large bubbles (≥10%) annotated with "ang 46%" etc.
    ...
    # ── Determine layout of bubbles at each node ──
    # For each row (feature) and each bin, sort emotions by proportion,
    # then alternate above/below the baseline
    for row in range(n_rows):
        ...
        for bi, (node_label, node_x) in enumerate(
            ...
            # Draw node marker on baseline
            ax.plot(node_x, 0, 'o', color='#4a90d9', markersize=7,
            ...
            # Sort by proportion (descending) so largest bubbles are centered
            items.sort(key=lambda x: -x[1])

            # Alternating vertical offsets
            base_offsets = [0.12, -0.12, 0.24, -0.24, 0.36, -0.36]

            # Per-node deterministic jitter
            rng = np.random.RandomState(42 + row * 10 + bi)
            x_jitters = rng.uniform(-0.06, 0.06, len(items))

            for position, (ei, prop) in enumerate(items):
                emo = emotions[ei]
                color = EMOTION_COLORS[emo]

                # Offset: alternate above/below
                y_off = base_offsets[position % len(base_offsets)]

                # Bubble size ∝ proportion
                bubble_size = MAX_BUBBLE * prop
                bubble_size = max(bubble_size, 30)  # minimum visible

                ax.scatter(
                    node_x + x_jitters[position], y_off,
                    s=bubble_size, c=[color], alpha=0.82,
                    edgecolors='white', linewidths=0.8, zorder=6,
                )

                # Annotation for large bubbles (>= 10%)
                if prop >= 0.10:
                    abbrev = EMOTION_ABBREV[emo]
                    # Place annotation slightly above bubble center
                    text_y = y_off + 0.04
        ...
        # Subtle left/right spines only
        for spine_name in ['top', 'bottom', 'left', 'right']:
            ax.spines[spine_name].set_visible(False)

    # ── Right-side color legend ──
    # Place legend as text + colored circles in the figure coordinate space
...
def main():
    ...
    # ── Plotting ──
    print(f"\n{'='*60}")
    print("Generating v4 figures (Tsangko-style layout)...")
