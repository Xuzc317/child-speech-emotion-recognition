...
The single most important result in this study is Exp4: a model trained on acted child speech (C-BESD, 92.78\% in-domain WA) achieves only \textbf{19.56\% WA} when evaluated zero-shot on spontaneous child speech (FAU Aibo)---a collapse of 73 percentage points, landing below the 25\% chance level.
...
The finding is consistent with distribution-shift diagnostics: the Fr\'{e}chet distance between C-BESD and FAU Aibo WavLM feature spaces is FD~=~8.50 (SMMD~=~0.378), confirming a substantial acoustic gap between acted and spontaneous child speech that no pooling architecture alone can bridge. This aligns with recent warnings in the SER literature that acted corpora produce ``dataset-solving'' models rather than emotion-understanding systems~\cite{wagner2023dawn}.
...
Our Attention--Prosody Correlation (APC) analysis yields $\text{APC}_{\text{wav}} = 0.718$ (Pearson $r$ between learned attention weights and frame-level RMS energy), confirming that prosody-guided attention locks onto physically interpretable acoustic events---energy bursts corresponding to emotional emphasis.
...
The 1.48pp advantage of self-attention exists \textit{only} on acted speech---a regime we have shown to be non-transferable to real-world spontaneous conditions (Exp4: 19.56\%).
...
Age shift (child $\rightarrow$ adult, C-BESD vs.\ IEMOCAP) & 7.20 & 58.67 \\
Style shift (acted $\rightarrow$ spontaneous, C-BESD vs.\ FAU, in-domain) & 8.50 & 66.36 \\
Style + language shift (C-BESD vs.\ FAU, zero-shot) & 8.50 & 19.56 \\
\bottomrule
\end{tabular}
\end{table}
