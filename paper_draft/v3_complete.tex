...
% v3 Complete Manuscript — INTERSPEECH / ICASSP 2027
%
% Key corrections from v2:
% 1. FAU Aibo: "spontaneous" replaced with "naturalistic child-robot interaction" throughout
% 2. Related Work restructured to four-topic synthesis (Corpora → SSL → Cross-Corpus → Prosody)
% 3. Table captions audited for self-containment; redundant discussion trimmed from Results
% 4. Acoustic feature bubble chart placeholder added before Finding 1 in Analysis
% 5. Canonical data preserved from v2: E1 real data, experiment counts, all citations
% ============================================================
...
spanning three datasets---acted children's speech (C-BESD, 6-class, 70 English--Telugu bilingual
children, ages unknown~\cite{rao2024cbesd}), naturalistic child-robot interaction speech
...
sensitivity, module ablation, and fine-tuning transfer regimes---67 unique
configurations (165 individual runs), of which the in-domain pooling baseline
(E1, 27 runs) is fully completed. Five findings emerge:
\textbf{(1)}~Fr\'{e}chet Distance (FD) between corpus-level WavLM feature
distributions exhibits a negative correlation with classification accuracy
across age, style, and enhancement dimensions (Spearman $\rho = -0.89$,
$n=5$ measured shift dimensions; FD increases correspond to WA decreases
...
outperforms on adult speech (PG$-$SA: $+0.62$pp on IEMOCAP), filling an
empirical gap in the literature where no prior work has systematically compared
these strategies across child and adult populations under matched budgets;
...
adult empirical knowledge does not transfer to pediatric populations;
\textbf{(4)}~mid-to-upper WavLM layers (L7--L11) receive highest fusion weights
on C-BESD (entropy~2.48 out of theoretical max~2.49, with FAU and IEMOCAP
pending); and
\textbf{(5)}~t-SNE visualization of pipeline-transformed penultimate features
reveals clear emotion cluster separation on acted children's speech, diffuse
class boundaries on naturalistic speech, and intermediate structure on adult
speech. We release all experimental configurations and
...
available child speech emotion corpora are acted rather than naturalistic,
...
been evaluated on acted child corpora but not on naturalistic child speech or
...
\item \textbf{A unified cross-corpus experimental framework} spanning three
  datasets (acted children, naturalistic children, acted adults) under strict
...
\subsection{Children's SER Corpora and Benchmarks}

The FAU Aibo corpus~\cite{batliner2011fau,steidl2009fau} is the largest
publicly available naturalistic child speech emotion corpus, recorded in a
child-robot interaction setting where 51 German children
(aged~10--13) interacted with a Sony Aibo robot dog. The INTERSPEECH
...
in English and Telugu from 70 children (aged 6--12), but critical metadata (child identity)
...
IEMOCAP~\cite{busso2008iemocap} serves as the standard acted adult benchmark,
providing a controlled adult contrast to the two child corpora.

Despite individual benchmarking efforts, the field lacks a systematic
cross-corpus child SER evaluation spanning acted, naturalistic, and adult speech
under matched experimental conditions. Our work fills this gap by constructing a
unified three-dataset framework with strict speaker-disjoint partitioning and 67
...
weighted sum. To our knowledge, systematic comparison of layer fusion strategies
has not been conducted on children's speech, motivating our three-way ablation
under matched training budgets.

\subsection{Cross-Corpus Transfer and Distribution Shift}
...
knowledge, this is the first application of FD as a diagnostic tool for
...
across five shift dimensions (age, style, enhancement, language, in-domain,
zero-shot).

\subsection{Acoustic-Prosodic Features in Emotion Recognition}

Children's vocal productions differ markedly from adults' in fundamental
frequency (mean F0 $\sim$265\,Hz vs.\ adult males 80--200\,Hz),
pitch variability, and articulatory
control~\cite{lee2007interspeech,tavares2010normative}. These acoustic
differences raise the question of whether prosodic priors---explicitly
encoding F0 and energy into pooling mechanisms---benefit children's SER more
than adults'.
...
each). We provide the first such cross-population assessment across three
pooling strategies, finding that the prosody prior is population-neutral rather
than child-specific.
...
categories (ANGER, DISGUST, FEAR, HAPPY, NEUTRAL, SAD) from 70 unique English--Telugu bilingual
children (aged 6--12), collected by Rao et al.\ (2024)
...
\paragraph{FAU Aibo (4-class).} The corpus contains 18,216
utterances from 51 German children (aged~10--13, two schools: Mont, Ohm)
recorded in a naturalistic child-robot interaction setting, where children
interacted with a Sony Aibo robot dog~\cite{batliner2011fau,
...
filtering, 9,794 utterances remain for 4-class classification.
...
\begin{table}[t]
\centering
\caption{Dataset statistics after unified preprocessing (16 kHz mono, peak normalization, 4 s truncation, MD5-hash speaker-disjoint 70/15/15 split).}
\label{tab:datasets}
...
FAU Aibo    & 4 & 18,216 & 51 children & 10--13 & naturalistic & German \\
IEMOCAP     & 4 & 9,794 & 10 adults & adult & acted & English \\
\bottomrule
\end{tabular}
\vspace{4pt}
...
\begin{table}[t]
\centering
\caption{E1: In-domain pooling comparison (WA\% / UAR\%, mean$\pm$std over 3 seeds, frozen WavLM Base + 12-layer weighted fusion + SE-MLP classifier). Bold = best per dataset.}
\label{tab:e1}
\begin{tabular}{llccc}
\toprule
...
\textit{Third,} the FAU naturalistic corpus exhibits a large WA--UAR gap
...
\begin{table}[t]
\centering
\caption{E3: Zero-shot cross-corpus transfer matrix (WA\% / UAR\%, seed 42, Self-Attention pooling, frozen WavLM Base). Train on source corpus, test directly on target without fine-tuning. Diagonal = in-domain E1 values.}
\label{tab:e3}
\begin{tabular}{lccc}
\toprule
...
The lowest transfer occurs on the naturalistic target (FAU: 20.51--21.32\% WA).
...
We also present qualitative visualizations of both raw acoustic features and
pipeline-transformed feature spaces.

\subsection{Acoustic Feature Distribution Across Corpora}

Before analyzing model-level diagnostics, we examine the raw acoustic
properties of the three datasets to establish a baseline for expected
distribution shifts. For each emotion class in each dataset, we discretize
frame-level F0 (fundamental frequency) and RMS energy into five bins and
render per-class bubble charts, where bubble size encodes the proportion of
frames falling within each (F0, energy) bin.

\textbf{Figure X: Per-emotion acoustic feature distributions across datasets.}
Bubble charts show the joint F0--energy distribution for each emotion class,
discretized into $5 \times 5$ bins (bubble size $\propto$ frame fraction).
Rows: C-BESD (acted children), FAU Aibo (naturalistic children), IEMOCAP
(acted adults). Columns: angry, happy, neutral, sad.

This visualization complements the t-SNE analysis in Finding~5
(\S\ref{ssec:tsne}): bubble charts capture the \textit{raw} acoustic
distribution before any model processing, while t-SNE reveals what the
pipeline \textit{learns} from those distributions. Together they bracket the
full data-to-decision trajectory.

Based on established pediatric acoustic norms (F0 $\sim$265\,Hz for children
vs.\ 80--200\,Hz for adult males~\cite{ncvs2025normative}) and our
corpus-level FD measurements (Table~\ref{tab:fd_wa}), we expect three
patterns: (i)~child corpora (C-BESD, FAU) will exhibit higher and more
variable F0 centroids than IEMOCAP across all emotion classes;
(ii)~naturalistic speech (FAU) will show broader, more diffuse
F0--energy distributions than acted speech (C-BESD, IEMOCAP), reflecting
uncontrolled recording conditions and spontaneous vocal behavior; and
(iii)~high-arousal emotions (angry, happy) will cluster at elevated F0 and
energy across all datasets, while neutral and sad will occupy lower-energy
regions. [图表待 Agent 2 生成]
...
\begin{table}[h]
\centering
\caption{FD--Accuracy correspondence across five distribution-shift dimensions. FD values from canonical WavLM feature-space computation. Spearman $\rho = -0.89$ ($n=5$, $p<0.05$).}
\label{tab:fd_wa}
\begin{tabular}{lccc}
\toprule
...
Style shift (naturalistic in-domain) & 8.50  & 67.05 & E1-05 (FAU SA) \\
...
across all five dimensions despite different underlying causes of distribution
...
\begin{table}[h]
\centering
\caption{Prosody-Guided vs.\ Self-Attention WA gap by population (3 seeds each, E1, $\sim$704K trainable parameters, frozen WavLM Base). Positive $\Delta$ = SA higher; negative $\Delta$ = PG higher.}
...
FAU Aibo & Children (naturalistic) & $+0.28$pp & PG $\approx$ SA \\
...
with limited linguistic and cultural diversity (English+Telugu, German, English).
