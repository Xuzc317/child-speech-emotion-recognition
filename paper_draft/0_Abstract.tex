...
First, zero-shot transfer from acted to spontaneous child speech collapses to \textbf{19.56\%} weighted accuracy (WA)---below the four-class chance level---despite \textbf{92.78\%} in-domain acted performance, confirming that distribution shift, not architecture, is the dominant failure mode (Fr\'{e}chet distance FD$=$8.50, C-BESD vs.\ FAU Aibo feature spaces).
...
Third, sacrificing only \textbf{1.48} percentage points on acted C-BESD buys interpretability: attention weights correlate strongly with frame-level energy (APC$_{\text{wav}}{=}0.718$), enabling clinically actionable explanations unavailable from black-box self-attention.
