...
children), spontaneous children's speech (FAU Aibo, 4-class, 51 German children
aged~10--13), and acted adult speech (IEMOCAP, 4-class, 10 English-speaking
...
\textbf{(4)}~mid-to-upper WavLM layers (L8--L9) receive highest fusion weights
across all three corpora, with near-uniform layer contribution (entropy~2.48
out of theoretical max~2.49). We release all experimental configurations and
...
This assumption is problematic for three reasons. First, children's vocal
productions differ systematically from adults': higher and more variable
fundamental frequency (mean F0 $\sim$265\,Hz vs.\ $\sim$120\,Hz for adult
males), immature articulatory control, and distinct prosodic
patterns~\cite{lee2007interspeech,tavares2010normative}. Second, the largest
available child speech emotion corpora are acted rather than spontaneous,
raising questions about ecological validity~\cite{batliner2011fau}. Third, and
...
representations~\cite{pasad2021layer} shows that mid-to-upper transformer
layers encode paralinguistic information, but these analyses are conducted on
...
for speech~\cite{le2024dist} but has not been applied to children's SER as a
diagnostic tool.
...
\item \textbf{A unified cross-corpus experimental framework} spanning three
  ...
  ($\text{A}+\text{E} \rightarrow \text{Angry}$,
  $\text{P} \rightarrow \text{Happy}$, $\text{R} \rightarrow \text{Sad}$) and
...
\item \textbf{67 controlled experimental configurations} (165 individual runs
  ...
  ablation, module ablation, and fine-tuning transfer. No prior study has run
  this scale of controlled comparison for children's SER.
...
\item \textbf{Four empirically supported findings:} (i)~FD--WA monotonic
  ...
  (iv)~mid-to-upper WavLM layer preference is consistent across populations.
...
energy as an explicit acoustic prior into the attention computation. Prior work
has evaluated these strategies on adult speech or small acted child corpora; we
provide the first systematic comparison across acted and spontaneous child
speech under matched parameter budgets (111,105 trainable parameters each).
...
70 children, but critical metadata (child identity) is embedded only in
filenames and has not been systematically extracted in prior work---we address
this through regex-based child-ID parsing.
...
corpus-level distribution shift metric in speech~\cite{le2024dist}. We apply FD
to WavLM frame-level features and use the resulting values as explanatory
variables for cross-corpus accuracy patterns across six shift dimensions (age,
style, enhancement, language, spontaneous in-domain, zero-shot).
...
children, recorded in preschool settings~\cite{sazali2023besd}. Child identity
...
recorded during child--robot interaction~\cite{batliner2011fau}. We use the
chunk-level 5-class annotation (A~=~Anger, E~=~Emphatic, N~=~Neutral,
P~=~Positive, R~=~Rest) and remap to a 4-class taxonomy by merging Anger and
Emphatic into \textit{Angry} (A+E $\rightarrow$ angry, 2,692 utterances),
treating Positive as \textit{Happy} (889), Neutral as \textit{Neutral} (10,967),
and Rest as \textit{Sad} (1,267). This remapping aligns FAU's label space with
the 4-class subsets of C-BESD and IEMOCAP. Speaker identity is verified against
the official \texttt{age.txt} metadata.
...
shift. A notable case is the style-shift dimension: the same FD value (8.50)
...
\textbf{population-neutral with a slight positive tendency}. This finding
...
suggest that WavLM's mid-to-upper layers already encode prosodic information
sufficiently for both populations, making the explicit prosody prior a mild
...
all available emotional speech data, regardless of population match, is
counterproductive when the target population is children. FD between the
augmentation source and target corpus provides a pre-augmentation diagnostic
for predicting expected degradation.
...
checkpoint assign highest values to layers 8--9 (1-based), with near-uniform
...
This pattern aligns with prior findings that WavLM layers 7--10 encode the most
transferable paralinguistic representations~\cite{pasad2021layer}, and extends
...
pattern, we hypothesize that mid-to-upper layer preference is universal across
populations, with corpus-specific variations in the exact argmax position.
...
(4)~mid-to-upper WavLM layers (L8--L9) receive highest fusion weights across
...
S.~Steidl et al.,
``{T}he {FAU Aibo} emotion corpus,''
\textit{Technical Report}, Univ. Erlangen-Nuremberg, 2009.
