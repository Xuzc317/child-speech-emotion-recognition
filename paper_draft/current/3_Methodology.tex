...
We extract frame-aligned F0 and RMS energy via
\texttt{librosa}~\cite{mcfee2015librosa} and linearly interpolate to $T$ frames.
F0 is normalized to $[0,1]$ via division by C7 (2093\,Hz, covering the upper
limit of children's vocal range~\cite{tavares2010normative}):
$\hat{f}_t = \text{F0}_t / 2093$. RMS energy is normalized per-sample to $[0,1]$
via its maximum: $\hat{e}_t = E_t / \max_{t} E_t$.

The raw 2-dimensional prosody vector $\mathbf{p}_t = [\hat{f}_t, \hat{e}_t]$ is
then projected to a 64-dimensional embedding through a two-layer MLP:

\begin{equation}
\mathbf{p}_t^{\text{emb}} = \text{MLP}_{\text{prosody}}(\mathbf{p}_t)
= \mathbf{W}_p^{(2)} \cdot \text{ReLU}(\mathbf{W}_p^{(1)} \mathbf{p}_t + \mathbf{b}_p^{(1)}) + \mathbf{b}_p^{(2)}
\end{equation}

where $\mathbf{W}_p^{(1)} \in \mathbb{R}^{64 \times 2}$,
$\mathbf{W}_p^{(2)} \in \mathbb{R}^{64 \times 64}$,
and a dropout layer is interposed before the second projection. This yields a
64-dimensional prosody embedding $\mathbf{p}_t^{\text{emb}} \in \mathbb{R}^{64}$
per frame (4,352 parameters). The embedding is concatenated with the SSL features:

\begin{equation}
\tilde{\mathbf{h}}_t = [\mathbf{h}_t; \mathbf{p}_t^{\text{emb}}] \in \mathbb{R}^{832}
\end{equation}

Attention scores are then computed from the augmented representation:
...
where $\mathbf{W}_1 \in \mathbb{R}^{128 \times 832}$, $\mathbf{v} \in \mathbb{R}^{128}$.
Crucially, prosody features influence only the attention weights; the pooled
output remains $\sum \alpha_t \mathbf{h}_t$, staying in the SSL space. The total
parameter count is 115,457, which differs from the self-attention pooling count
(111,105) by exactly the 4,352 parameters of the prosody embedding MLP. For
ablation fairness, we control for this difference by (a)~keeping the
\textit{architecture} of the attention head identical except for the input
dimension, and (b)~reporting the contribution of the 64-dimensional embedding
explicitly in module ablation (E6, \S\ref{ssec:module_ablation}).

\paragraph{Design rationale.} The two-layer projection from 2\,d to 64\,d is
deliberately small: it provides enough capacity to encode the joint
distribution of F0 and energy (their linear and non-linear interactions---e.g.,
high F0\,$+$\,high energy $\rightarrow$ anger vs.\ high F0\,$+$\,low energy
$\rightarrow$ sadness) without over-parameterizing a low-dimensional signal.
No batch normalization is applied, preserving the physically meaningful
$[0,1]$ range across samples.
