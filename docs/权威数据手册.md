...
### 1.1 C-BESD 多随机种子（3-seed）

| 实验 | Seeds | Test WA mean±std | Test UAR mean±std |
|------|-------|------------------|-------------------|
| Exp1 Self-Attn | 42, 123, 456 | **94.97% ± 1.66%** | **94.97% ± 1.66%** |
| Exp2 Prosody | 42, 123, 456 | **95.10% ± 2.78%** | **95.10% ± 2.76%** |
| Δ (SA − PG) | | −0.13pp (n.s.) | |
...
|-----|------|---------|----------|------------|
| Exp1 | 42 | 92.78% | 92.79% | 30 |
| Exp1 | 123 | 96.80% | 96.80% | 28 |
| Exp1 | 456 | 95.32% | 95.32% | 8 |
| Exp2 | 42 | 91.30% | 91.35% | 10 |
| Exp2 | 123 | 96.12% | 96.12% | 20 |
| Exp2 | 456 | 97.88% | 97.88% | 53 |

### 1.2 IEMOCAP 多随机种子（2-seed，s456 val=0 不可用）

| 实验 | Seeds | Test WA mean±std |
|------|-------|------------------|
| Exp3 Self-Attn | 42, 123 | **75.96% ± 7.94%** |
| Exp3 Prosody | 42, 123 | **66.23% ± 7.56%** |
| Δ (SA − PG) | | +9.73pp（大方差） |

> ⚠️ IEMOCAP 仅 10 说话人，70/15/15 hash split 导致 s456 val=0。论文需标注此限制。
...
| 指标 | 1样本值 | 全量值（540样本） | 来源 |
|------|---------|------------------|------|
| APC_wav | 0.718 | **0.7406 ± 0.1087** | `apc_full_test.json` |
| APC_delta | −0.144 | **−0.0450 ± 0.0891** | 同上 |
| Layer argmax (0-based) | 8 | — | `layer_weights.json` |
| Layer argmax (1-based) | 9 | — | 同上 |
| Layer entropy | 2.484 | — | 同上 |
| 计算 checkpoint | `checkpoints/exp2_prosody_guided/best_model.pt` | Exp2, epoch 10 |
| 评估样本 | C-BESD test, seed=42, 首条 | 全 540 条 test | — |

> ✅ APC 已更新为全量 540 样本均值。APC_delta 为负值 → Prosody Pooling 降低了注意力与声学韵律的冗余相关。
...
| 参数 | 值 |
|------|-----|
| SSL backbone | WavLM Base (wavlm-base-sv), frozen |
...
| 项目 | 说明 |
|------|------|
| APC | ✅ 已更新为全量 540 样本（`apc_full_test.json`） |
